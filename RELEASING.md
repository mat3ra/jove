...
`github:mat3ra/jove#branch` (which has no way to build `dist/` on install), CI publishes
...
```text
https://github.com/mat3ra/jove/releases/download/wip-<short-sha>/jove.tgz
```
...
```bash
npm run mat3ra:install -- jove wip-<short-sha>
```

(`web-app/scripts/mat3ra-install.sh` — installs `@mat3ra/jove@<release-url>` with
